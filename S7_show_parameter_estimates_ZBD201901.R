##===========================  S7 PARAMETER ESTIMATES OF FITTED MODELS      =====================================##
...
##=============================================================================================================##
library(Hmsc); library(writexl); library(colorspace)
library(corrplot); library(writexl)

# THIS SCRIPT COMPUTES PARAMETER ESTIMATES FOR FITTED HMSC MODELS BASED ON CODE FROM THE THE BOOK
...
filename <- paste("models/Final/models_thin_", as.character(thin),
...
filename <- paste("figures/parameter_estimates.pdf")

# Save outputs as pdf and excel files
pdf(file = filename)
for(j in 1:nm){
  m <- models[[j]]
...
for(j in 1:nm){
  ...
  show.sp.names <- (is.null(m$phyloTree) && m$ns<=80) 
  plotBeta(m, post=postBeta, supportLevel = 0.90,param="Support",
...
for(j in 1:nm){
  if(m$nt>1){
    ...
    plotGamma(m, post=postGamma, supportLevel = 0.75, param="Support",
              covNamesNumbers = c(TRUE,FALSE),
...
for(j in 1:nm){
  ...
  for (r in 1:m$nr){
    ...
    toPlot <- (OmegaCor[[r]]$support>supportLevel) + (OmegaCor[[r]]$support<(1-supportLevel)) * OmegaCor[[r]]$mean
    if(m$ns>90){
    ...
    corrplot(toPlot[plotOrder,plotOrder], method = "color",
             type = "lower", diag = F,bg = "white",tl.col = "black",tl.cex = 1.0, tl.srt = 90, tl.offset = 1,
             col=colorRampPalette(c("blue","white","red"))(21),
             mar=c(0,0,1,0),
             main=NULL,cex.main=0.8)
    ...
    colnames(ne)[1] <- ""
    vals <- list("Posterior mean"=me,"Pr(x>0)"=po,"Pr(x<0)"=ne)
    filename <- paste0("figures/parameter_estimates_Omega_",modelnames[[j]],"_",names(m$ranLevels)[[r]],".xlsx")
    writexl::write_xlsx(vals,path = filename)
  }
}
dev.off()

# can see the figures in both proportional and raw format (i.e the unexplained variation)

# species betaplots (relatoinships with env) - intercept interpreted as rarity (if negative = rare)
